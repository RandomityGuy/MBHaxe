...
class GuiBitmapBorderCtrl extends GuiControl {
	...
	public override function render(scene2d:Scene, ?parent:Flow) {
		...
		br.setPosition(renderRect.extent.x - tr.tile.width - 1, renderRect.extent.y - bl.tile.height);
		...
		right.setPosition(renderRect.extent.x - tr.tile.width - 1, tl.tile.height);
